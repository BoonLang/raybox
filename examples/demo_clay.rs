//! Demo 5: 3D Clay Tablet with Relief Text
//! Text carved into a clay slab
...
impl Renderer {
    fn new(window: Arc<Window>) -> Result<Self> {
        ...
        let shader_module = sdf_clay::create_shader_module_embed_source(&device);

        // Camera looking at the tablet from above
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.renderer.is_none() {
            let window_attrs = Window::default_attributes()
                .with_title("Demo 5: Clay Tablet (A/D: rotate, W/S: zoom, Q/E: tilt)")
