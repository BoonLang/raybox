...
//!
//! Run with: cargo run --example demo_text_shadow --features windowed
//! Screenshot: cargo run --example demo_text_shadow
...
fn build_shadow_text_layout(atlas: &VectorFontAtlas) -> Vec<GpuCharInstance> {
    ...
    // Generate text content with title and Lorem ipsum
    let full_text = format!(
        "VECTOR SDF TEXT\n\n{}",
        format!("{} {} {}", LOREM, LOREM, LOREM)
    );

    let scale = 0.12; // Character scale for cleaner rendering
...
#[cfg(feature = "windowed")]
const DEMO_TITLE: &str = "Demo 6: 3D Text Shadow";
...
fn run_windowed() -> Result<()> {
    ...
    impl Renderer {
        fn new(window: Arc<Window>) -> Result<Self> {
            ...
            // Camera config: positioned in front of text, looking at it
            ...
            // Create pipeline
            let shader_module =
                shader_bindings::sdf_text_shadow_vector::create_shader_module_embed_source(&device);
        ...
        fn resumed(&mut self, event_loop: &ActiveEventLoop) {
            if self.renderer.is_none() {
                let window_attrs = Window::default_attributes()
                    .with_title(input::demo_title(6, "3D Text Shadow"))
...
fn run_headless_screenshot() -> Result<()> {
    println!("Capturing headless screenshot of Floating 3D Text with Shadows...");
    ...
    // Create pipeline
    let shader_module = shader_bindings::sdf_text_shadow_vector::create_shader_module_embed_source(&device);
    ...
    image::save_buffer(
        "output/demo_text_shadow.png",
        &image_data,
        width,
        height,
        image::ColorType::Rgba8,
    )?;
    println!("Screenshot saved to output/demo_text_shadow.png");
